...
// clobbers the cursor
fn peek_past_metadata<'a, TkIter: Iterator<Item = Token>>(
  tkiter: &'a mut PeekMoreIterator<TkIter>,
) -> &'a Token {
  tkiter
...
fn parse_prefix_op<TkIter: Iterator<Item = Token>>(
  ...
  if let Some((op, range, metadata)) = operator_fn(tkiter, dlogger) {
    ...
    Expr {
      ...
      kind: ExprData::UnaryOp { op, operand },
      metadata,
    }
  } else {
...
fn parse_label<TkIter: Iterator<Item = Token>>(
  tkiter: &mut PeekMoreIterator<TkIter>,
  _: &mut DiagnosticLogger,
) -> Option<Label> {
  let Token { range, kind } = tkiter.peek().unwrap();
  if let Some(TokenKind::Label(value)) = kind {
    let label = Label {
      range: *range,
      value: value.clone(),
    };
...
fn parse_identifier<TkIter: Iterator<Item = Token>>(
  tkiter: &mut PeekMoreIterator<TkIter>,
  _: &mut DiagnosticLogger,
) -> Option<Identifier> {
  let Token { range, kind } = tkiter.peek().unwrap();
  if let Some(TokenKind::Identifier(value)) = kind {
...
fn parse_exact_struct<TkIter: Iterator<Item = Token>>(
  ...
  let metadata = get_metadata(tkiter);
  assert!(tkiter.peek().unwrap().kind == Some(TokenKind::BraceLeft));
...
fn parse_exact_group<TkIter: Iterator<Item = Token>>(
  ...
  let metadata = get_metadata(tkiter);
  assert!(tkiter.peek().unwrap().kind == Some(TokenKind::ParenLeft));
  let Token { range: lrange, .. } = tkiter.next().unwrap();
...
fn parse_exact_reference<TkIter: Iterator<Item = Token>>(
  ...
  let metadata = get_metadata(tkiter);
  let identifier = parse_identifier(tkiter, dlogger).unwrap();
  Expr {
    metadata,
    range: identifier.range,
    kind: ExprData::Reference(Some(identifier)),
  }
}
...
fn parse_exact_ret<TkIter: Iterator<Item = Token>>(
  ...
  let label = parse_label(tkiter, dlogger);
  let body = Box::new(parse_expr(tkiter, dlogger));

  Expr {
    metadata,
    range: union_of(ret_tk.range, body.range),
    kind: ExprData::Ret { label, body },
  }
}
...
fn parse_exact_defer<TkIter: Iterator<Item = Token>>(
  ...
  Expr {
    ...
    kind: ExprData::Defer { label, body },
  }
}

// parses a if or panics
fn parse_exact_if<TkIter: Iterator<Item = Token>>(
  tkiter: &mut PeekMoreIterator<TkIter>,
  dlogger: &mut DiagnosticLogger,
) -> Expr {
  let metadata = get_metadata(tkiter);
  assert!(tkiter.peek_nth(0).unwrap().kind == Some(TokenKind::If));
  let if_tk = tkiter.next().unwrap();
  let cond_expr = Box::new(parse_term(tkiter, dlogger));
  let then_tk = tkiter.next().unwrap();
  if then_tk.kind != Some(TokenKind::Then) {
    dlogger.log_unexpected_token_specific(then_tk.range, Some(TokenKind::Then), then_tk.kind);
  }
  let then_expr = Box::new(parse_term(tkiter, dlogger));
  let else_tk = tkiter.next().unwrap();
  if else_tk.kind != Some(TokenKind::Else) {
    dlogger.log_unexpected_token_specific(else_tk.range, Some(TokenKind::Else), else_tk.kind);
  }
  let else_expr = Box::new(parse_term(tkiter, dlogger));

  Expr {
    metadata,
    range: union_of(if_tk.range, else_expr.range),
    kind: ExprData::IfThen {
      cond_expr,
      then_expr,
      else_expr,
    },
...
fn parse_exact_case<TkIter: Iterator<Item = Token>>(
  ...
  Expr {
    ...
    kind: ExprData::CaseOf { expr, cases },
  }
}
...
fn parse_exact_label<TkIter: Iterator<Item = Token>>(
  ...
  let metadata = get_metadata(tkiter);
  let label = parse_label(tkiter, dlogger).unwrap();
  let body = Box::new(parse_term(tkiter, dlogger));
  Expr {
    metadata,
    range: union_of(label.range, body.range),
    kind: ExprData::Label { label, body },
  }
}
...
fn parse_exact_bool_type<TkIter: Iterator<Item = Token>>(
  ...
  parse_exact_simple(TokenKind::BoolType, ExprData::BoolType)(tkiter, dlogger)
}
...
fn parse_exact_string<TkIter: Iterator<Item = Token>>(
  ...
  } = tkiter.next().unwrap()
  {
    Expr {
      metadata,
      range,
      kind: ExprData::String { value, block },
    }
  } else {
    unreachable!();
  }
...
fn parse_exact_int_type<TkIter: Iterator<Item = Token>>(
  ...
  parse_exact_simple(TokenKind::IntType, ExprData::IntType)(tkiter, dlogger)
}
...
fn parse_exact_int<TkIter: Iterator<Item = Token>>(
  ...
  {
    let (sign, values) = value.to_u32_digits();
    Expr {
      metadata,
      range,
      kind: ExprData::Int {
        positive: sign != Sign::Minus,
        values,
      },
    }
  } else {
...
fn parse_exact_real_type<TkIter: Iterator<Item = Token>>(
  ...
  parse_exact_simple(TokenKind::RealType, ExprData::RealType)(tkiter, dlogger)
}
...
fn parse_exact_real<TkIter: Iterator<Item = Token>>(
  ...
  {
    let (nsign, numerator) = value.numer().to_u32_digits();
    let (dsign, denominator) = value.denom().to_u32_digits();
    Expr {
      metadata,
      range,
      kind: ExprData::Real {
        numerator,
        denominator,
        positive: nsign * dsign == Sign::Minus,
      },
    }
  } else {
    unreachable!();
...
fn parse_exact_this<TkIter: Iterator<Item = Token>>(
  ...
  parse_exact_simple(TokenKind::This, ExprData::This)(tkiter, dlogger)
}
...
fn parse_exact_ignore<TkIter: Iterator<Item = Token>>(
  ...
  parse_exact_simple(TokenKind::Ignore, ExprData::BindIgnore)(tkiter, dlogger)
}
...
fn parse_exact_splat<TkIter: Iterator<Item = Token>>(
  ...
  parse_exact_simple(TokenKind::Splat, ExprData::BindSplat)(tkiter, dlogger)
}
...
fn parse_exact_nil_type<TkIter: Iterator<Item = Token>>(
  ...
  parse_exact_simple(TokenKind::NilType, ExprData::NilType)(tkiter, dlogger)
}
...
fn parse_exact_never_type<TkIter: Iterator<Item = Token>>(
  ...
  parse_exact_simple(TokenKind::NeverType, ExprData::NeverType)(tkiter, dlogger)
}
...
fn decide_term<TkIter: Iterator<Item = Token>>(
  ...
  match *tkkind {
    ...
    TokenKind::If => Some(parse_exact_if::<TkIter>),
...
fn parse_apply_operators<TkIter: Iterator<Item = Token>>(
  ...
  parse_r_binary_op(tkiter, dlogger, parse_suffix_operators, |tkiter, _| {
    if let Token {
      kind: Some(kind),
      range,
    } = peek_past_metadata(tkiter)
