...
static JsonElem typeExprJson(TypeExpr *tep, Arena *ja) {
  ...
  case TEK_Struct: {
    ...
    ptrs[3] = KVJson("struct_kind", strJson(structKind));
  ...
  // Embed array
    ...
    ptrs[4] =
        KVJson("trailing_comma", boolJson(tep->tupleExpr.trailing_comma));
    break;
  }
  case TEK_Fn: {
    ptrs_len = 6;
...
static JsonElem valueExprJson(ValueExpr *vep, Arena *ja) {
  ...
  case VEK_StringLiteral: {
    ...
    ptrs[3] = KVJson("value", strJson(internArena(vep->stringLiteral.value, ja)));
    break;
  }
...
JsonElem stmntJson(Stmnt *sp, Arena *ja) {
  ...
  case SK_FnDecl: {
    ...
    ptrs[3] = KVJson("name", strJson(internArena(sp->fnDecl.name, ja)));
    // Embed array
