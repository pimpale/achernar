...
impl DiagnosticLogger {
    ...
    pub fn log_unexpected_binop_in_pattern_struct(
        &mut self,
        range: Range,
    ...
    pub fn log_binop_only_in_pattern(&mut self, range: Range, kind: &ast::BinaryOpKind) {
        self.log(Diagnostic {
             range,
             severity: Some(DiagnosticSeverity::Error),
             code: Some(NumberOrString::Number(13)),
    ...
    pub fn log_unop_only_in_pattern(&mut self, range: Range, kind: &ast::UnaryOpKind) {
        self.log(Diagnostic {
        range,
        severity: Some(DiagnosticSeverity::Error),
    ...
    pub fn log_repeat_splat_in_pattern_struct(&mut self, range:Range) {
        ...
        source: self.source.clone(),
        message: format!(
          "Only one splat is allowed in a pattern struct.",
          kind.to_string()
        ),
